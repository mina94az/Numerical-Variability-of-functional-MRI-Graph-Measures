...
uncertainty expressions used in the manuscript. It writes only the expanded
machine-readable ``table1_all_formulae_lookup_values.csv`` files; it does not
create LaTeX or PDF files.
...
DEFAULT_INPUTS = {
    "combined": SCRIPT_DIR / "NPVR_table.csv",
    "PD": SCRIPT_DIR / "NPVR_tablePD.csv",
    "HC": SCRIPT_DIR / "NPVR_tableHC.csv",
}
...
def make_combined_lookup_table(
    ...
    for metric in METRIC_ORDER:
        ...
        npvr_header = " & ".join(format_number(npvr[t], digits) for t in thresholds)
        rows.append(
            r"\rowcolor[gray]{0.92} \multicolumn{3}{l}{" + metric_label
            + r"} & \multicolumn{1}{r}{\textbf{NPVR values}} & "
            + npvr_header + r" \\"
        )
        rows.append(r"\midrule")

        for test_name, table_key, statistic_name, n_independent in specs:
            ...
            for row_number, (index, values) in enumerate(pivot_rows):
                ...
                # Repeat the graph metric and statistical test at the start
                # of every statistic group.  This keeps long lookup tables
                # understandable when a group continues on another page or
                # is read without tracing back to the start of a test block.
                starts_statistic_group = (
                    row_number == 0 or statistic_value != previous_statistic
                )
                graph_metric = metric_label if starts_statistic_group else ""
                statistical_test = test_name if starts_statistic_group else ""
    ...
    threshold_header = " & ".join(threshold_header_item(t) for t in thresholds)
    header = (
        r"\textbf{Graph metric} & \textbf{Statistical test} & \textbf{Statistic} & "
        r"\textbf{$n$} & " + threshold_header + r" \\"
    )
    numeric_column = r">{\raggedleft\arraybackslash}p{0.0695\linewidth}"
    column_specification = (
        r"p{0.14\linewidth} p{0.15\linewidth} p{0.07\linewidth} "
...
def write_outputs(
    ...
    """Write the deterministic expanded lookup CSV."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate expanded NPVR uncertainty lookup CSVs only.",
